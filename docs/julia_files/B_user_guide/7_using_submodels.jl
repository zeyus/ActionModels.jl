...
# When making complex action models, it can be useful to have a type with it's own set of methods that can be run inside the model.
# This can also be useful when one wants to pre-create part of the model outside of the model fitting function.
# However, this also adds some requirements to the model structure, in order to avoid type-instability.
# In ActionModels, this is ensured by using creating a Submodel type and passing it to the ActionModel constructor.
# In this section, we will demonstrate how to do this, using a simple example of a Rescorla-Wagner model.
# The Rescorla-Wagner model is a simple reinforcement learning model that learns expectations about observations based on previous observations.
...
# This needs to be a mutable struct, as it will be updated during the model fitting process.
...
# The approriate types (which here is only the Float64 type) should be passed to the attributes constructor.
...
# ## Supporting parameter and state API
# Finally, we need to define methods for getting and setting states and parameters of the submodel.
...
# Then we create functions for getting the parameters and states from the attributes
...
# Note that the value passed should be a subtype of Real, or a subtype of AbstractArray{<:Real} if the parameter or state is multivariate.
# If the parameter or state name is not found, these methods should return an AttributeError, which will be handled by the higher level function.
...
# And finally, we define a method for getting the history 

# ## Adding custom functionality for the submodel attributes
# Now we have a functioning submodel which can integrate with the ActionModels API.
...
# The submodel attributes can be found in the `submodel` of the `ModelAttributes` object.
...
# We still need to define the action noise parameter, since it is not part of the Rescorla-Wagner submodel.
